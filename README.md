...
This project aims to create a website that can be used to log issues that typically occur in assignments, 
track their status, priority level and progress, and allow their details to be edited. It will also allow 
for member management for projects with multiple members as well as project management, for multiple 
projects.
...
##Data Schema:
• Issue: {id, summary, description, identifiedBy, dateIdentified, project, status, priority, assignedTo, 
  targetResolution, actualResolution, resolutionSummary}
...
• View Dashboard-
All issues appear in the dashboard with status and priority badges. A filter can also be applied here to 
only show certain statuses (Open, Resolved, Overdue). 
...
1) Create Issue-
   The user is taken to the “Create New Issue” screen where the user can:
      ...
      b. Enter Issue Details-
         Enter all the required information for the issue (summary, description, identifiedBy, 
	     dateIdentified, project, status, priority, assignedTo, targetResolution, actualResolution, 
	     resolutionSummary).
